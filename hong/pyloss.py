...
class EuclideanLossLayer(nn.Module):
    """
...
#                print 'neg is more'
            ...
            zip_loss = list(zip(flatten_loss, [p for p in range(len(flatten_loss))]))
...
#     def reshape(self, bottom, top):
    def forward(self, *bottom):
        bottom = [d.numpy() for d in bottom]
        self.scale0_data = bottom[0]
        GV.xxx = bottom
#        das
        saliency_map = bottom[1][:,0:2]
        self.labels = bottom[2]
...
#         top[0].data[...] = self.loss
        self.loss=torch.tensor(self.loss, dtype=bottom[0].dtype)
        return self.loss
