...
class SimpleNN(nn.Module):
    def __init__(self, split):
        ...
        pad = 1
#         self.block_nums = 1
#         self.block_nums = self.block_nums - 1
...
#         data, label = (torch.tensor(i, dtype=torch.double) for d in self.input_layer())
        data, label=self.get_input()
        return self.forward(data), label
    
    def get_input(self):
        return (torch.tensor(d, dtype=torch.double) for d in self.input_layer())
