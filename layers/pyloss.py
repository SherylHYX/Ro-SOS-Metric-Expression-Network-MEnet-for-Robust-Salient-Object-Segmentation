...
#            saliency_map = self.scale0_data
            ...
            pos_sample_index = random.sample(np.arange(pos_index[0].__len__()).tolist(), sample_num)
...
#                GV.final = (GV.final - GV.final.min()) * 1. / (GV.final.max() - GV.final.min()) * 255.0
            ...
            plt.imshow(GV.image)
            # plt.imshow(GV.trans_data[0].transpose(1,2,0))
            # GV.trans_data,GV.trans_label
            plt.subplot(142)
            plt.imshow(scale0_labels[0])
